...
extern NSString * _Nonnull const kBSUserTokenNotificationUserInfoKeyManagerToContinue;

@class CTAPIBaseManager;

// 在调用成功之后的params字典里面，用这个key可以取出requestID
static NSString * _Nonnull const kCTAPIBaseManagerRequestID = @"kCTAPIBaseManagerRequestID";
...
@property (nonatomic, assign) BOOL shouldDisableAutoLogin;
...
@property (nonatomic, strong) NSString * _Nullable currentPageName;
 ...
 */
- (NSDictionary *_Nonnull)reformParams:(NSDictionary *_Nullable)params;
- (void)cleanData;

- (void)successedOnCallingAPI:(CTURLResponse *_Nullable)response;
- (void)failedOnCallingAPI:(CTURLResponse *_Nullable)response withErrorType:(CTAPIManagerErrorType)errorType;
@end
